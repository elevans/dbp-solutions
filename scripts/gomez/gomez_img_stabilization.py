...
def run():
    ...
    stabilized_image_ids = []

    for id in init_image_ids:
        # get an open image
        tmp_init_imp = WindowManager.getImage(id)

        # get image dims and check the number of channels
        tmp_dims = tmp_init_imp.getDimensions()
        if tmp_dims[2] > 1:
            ...
            for v in tmp_open_image_ids:
                if v not in init_image_ids and v not in stabilized_image_ids:
            ...
            for v in tmp_open_image_ids:
                if v not in init_image_ids and v not in stabilized_image_ids:
                    stabilized_image_ids.append(v)

            # hide input images
            for v in init_image_ids:
                WindowManager.getImage(v).hide()
        else:
            image_stabilization(tmp_init_imp)
            tmp_init_imp.show()
            stabilized_image_ids.append(id)
